...
function getAbsensi() {
  try {
    ...
    if (s) { sessionStorage.setItem('absensi_backup', s); return JSON.parse(s); }
...
function showFormKaryawan(id) {
  ...
  if (id) {
    ...
    if (u) {
      ...
      if (document.getElementById('fJamPulang')) document.getElementById('fJamPulang').value = u.jamPulang || '17:00';
    }
  }
...
function hapusKaryawan(id) {
  ...
  // Hapus semua data absensi milik karyawan ini
...
function simpanKaryawan(e) {
  ...
  var jamPulang = document.getElementById('fJamPulang') ? document.getElementById('fJamPulang').value : '17:00';
  var users    = getUsers();

  if (id) {
    users = users.map(function(u) {
      if (u.id === parseInt(id)) {
        var updated = { id:u.id, username:username, nama:nama, role:u.role, divisi:divisi, jamMasuk:jamMasuk, jamPulang:jamPulang, password:u.password };
    ...
    users.push({ id:newId, username:username, password:password, nama:nama, role:'karyawan', divisi:divisi, jamMasuk:jamMasuk, jamPulang:jamPulang });
  }
  saveUsers(users);
  closeModalKaryawan();
...
function backupData() {
  ...
  a.download = 'backup_fatih_' + new Date().toISOString().slice(0,10) + '.json';
  a.click();
}
...
function restoreData(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (data.absensi) localStorage.setItem('absensi', JSON.stringify(data.absensi));
