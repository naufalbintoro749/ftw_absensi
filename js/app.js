...
if (!localStorage.getItem('users')) {
  ...
  // Hanya tambahkan default user yang belum ada sama sekali (by id)
  // JANGAN timpa data yang sudah diedit/ditambah admin
  try {
    var stored = JSON.parse(localStorage.getItem('users'));
    var changed = false;
    USERS_DEFAULT.forEach(function(def) {
      var exists = stored.some(function(u){ return u.id === def.id; });
      if (!exists) { stored.push(def); changed = true; }
    });
    if (changed) localStorage.setItem('users', JSON.stringify(stored));
  } catch(e) {
    localStorage.setItem('users', JSON.stringify(USERS_DEFAULT));
  }
}
